...
public class DedikastController implements Initializable {
     ...
     */
  @FXML  public List<MediaPlayer> videoModel;   
  @FXML  public  ObservableList<MediaPlayer> loadedDataVideo ;
 @FXML   public  TableView <MediaPlayer> listVideo ;
 @FXML   public final String LIST_VIDEO_ID = "list-video";
 @FXML   public String TITRE_COLUMN_NAME = "titre";
 @FXML  public  String ARTISTE_COLUMN_NAME = "artist";
   
 
 @FXML  public List<MediaPlayer> soundModel;   
  @FXML  public  ObservableList<MediaPlayer> loadedDataAudio ;
 @FXML    public  TableView <MediaPlayer> listAudio ;
 ...
 @FXML  public List<MediaPlayer> bookModel;   
  @FXML  public  ObservableList<MediaPlayer> loadedDataLivre ;
 @FXML    public  TableView <MediaPlayer> loadedListLivre ;
 @FXML   public  String TITRE_LIVRE_COLUMN_NAME = "titre";
  @FXML public  String ARTISTE_LIVRE_COLUMN_NAME = "auteur";
 @FXML   public String GENRE_LIVRE_COLUMN_NAME = "genre";
     @FXML public  String ANNEE_LIVRE_COLUMN_NAME = "année";
 
 
 
 
 @FXML   public final String MEDIA_VIEW_ID ="media-view";
  @FXML   public Stage PRIMARY_STAGE;
 @FXML    public String VIS_CONTAINER_ID; 
  @FXML   public ChangeListener<Duration> progressListener;
   @FXML  public final String SEEK_POS_SLIDER_ID="seek-pos-slider";
    @FXML public InvalidationListener StatusListener;
    
    
 @FXML   public final  Image buttonPlayImage = new Image(Dedikast.class.getResourceAsStream("/assets/play.png"));
  @FXML  public final  Image buttonPauseImage = new Image(Dedikast.class.getResourceAsStream("/assets/pause.png"));
  @FXML  public final  Image buttonBackwardImage = new Image(Dedikast.class.getResourceAsStream("/assets/backWard.png"));
 @FXML   public final  Image buttonForwardImage = new Image(Dedikast.class.getResourceAsStream("/assets/forward.png"));
 @FXML   public final  Image buttonLectureAleatoireImage = new Image(Dedikast.class.getResourceAsStream("/assets/buttonLectureAleatoire1.png"));
 @FXML   public final  Image buttonLectureContinueImage = new Image(Dedikast.class.getResourceAsStream("/assets/buttonLectureContinue1.png"));
 @FXML   public final  Image buttonLectureContinueImage11 = new Image(Dedikast.class.getResourceAsStream("/assets/buttonLectureContinue11.png"));
    ...
    private final String TOOL_BAR="tool-bar";
    private final String defaultLibrary= "defaul-library";
    private final String TAB_AUDIO_ID="tab-audio";
    private final String TAB_VIDEO_ID="tab-video";
...
//instantiate  stage
   ...
   PRIMARY_STAGE.centerOnScreen();
    ...
    scene.getStylesheets().addAll(getClass().getResource("dedikast.css").toExternalForm());
...
backward.setOnMousePressed((MouseEvent mouseEvent) -> {
    });
...
foreward.setOnMousePressed((MouseEvent mouseEvent) -> {
    });    
...
private Node createLibraryPanel(){
    ...
    List<MediaPlayer> loadedVideoModel = new VideoModel().getVideoModel();
final ObservableList<MediaPlayer> loadedDataVideo;
      loadedDataVideo = FXCollections.observableArrayList(loadedVideoModel);
    final TableView <MediaPlayer> loadedListVideo;
      loadedListVideo = new TableView(loadedDataVideo);
    
   TableColumn<MediaPlayer, String> titreColumn = new TableColumn< >("Titre");   
   TableColumn<MediaPlayer, String>  yearColumn = new TableColumn< >("Année");


       
       
       // add a metadataTable for meta data display
    loadedListVideo.setStyle("-fx-font-size: 13px;"+"-fx-background-color:green;");
    loadedListVideo.setId(LIST_VIDEO_ID);
    ...
    loadedListVideo.setItems(loadedDataVideo);
    loadedListVideo.getColumns().addAll(titreColumn,yearColumn);
    
    
    
    loadedListVideo.getSelectionModel().selectedItemProperty().addListener((ObservableValue<? extends MediaPlayer> ov, MediaPlayer old_val, MediaPlayer new_val) -> {
...
// read selected item on 2Click
   ...
   loadedListVideo.setOnMouseClicked((MouseEvent event) -> {
        if (event.getClickCount() == 2) {
            MediaPlayer mediaPlayer ;
            mediaPlayer = loadedListVideo.getSelectionModel().getSelectedItem() ;
            playMedia(mediaPlayer );
            Scene scene1 = PRIMARY_STAGE.getScene();
            scene1.lookup("#"+VIS_CONTAINER_ID).setVisible(false);
        }
    });
   ...
   final List <MediaPlayer> loadedSoundModel =  new SoundModel().getSoundModel(); 
 
     
    final ObservableList <MediaPlayer>    loadedDataAudio = FXCollections.observableArrayList(loadedSoundModel);
    final   TableView<MediaPlayer > loadedListAudio = new TableView(loadedDataAudio);
    ...
    TableColumn<MediaPlayer, String>  genreAudioColumn = new TableColumn<>("Genre");
       ...
       // add a metadataTable for meta data display
    loadedListAudio.setStyle("-fx-font-size: 13px;"+"-fx-background-color:green;");
    loadedListAudio.setId(LIST_AUDIO_ID);
   ...
   genreAudioColumn.setCellValueFactory(new PropertyValueFactory<>(GENRE_AUDIO_COLUMN_NAME));
    ...
    loadedListAudio.setItems(loadedDataAudio);
    loadedListAudio.getColumns().addAll(titreAudioColumn,artisteAudioColumn,albumAudioColumn,genreAudioColumn);
    
    loadedListAudio.getSelectionModel().selectedItemProperty().addListener((ObservableValue<? extends MediaPlayer> ov, MediaPlayer old_val, MediaPlayer new_val) -> {
        MediaPlayer   mediaPlayer = new_val;
    });
   
     
    
    loadedListAudio.setOnMouseClicked((MouseEvent event) -> {
        if (event.getClickCount() == 2) {
            MediaPlayer mediaPlayer ;
            mediaPlayer = loadedListAudio.getSelectionModel().getSelectedItem();
            playMedia(mediaPlayer );
            Scene scene1 = PRIMARY_STAGE.getScene();
 ...
 vitrine.setId(defaultLibrary);
    ...
    video.setContent(loadedListVideo);
        
    audio.setContent(loadedListAudio);
    ...
    VBox.setVgrow(loadedListVideo,Priority.ALWAYS);
    VBox.setVgrow(loadedListAudio,Priority.ALWAYS);
...
// create a bar to hide and show
    ...
    controlButton.setOnAction((ActionEvent actionEvent) -> {
        ...
        showSideBar.onFinishedProperty().setValue((EventHandler<ActionEvent>) (ActionEvent event) -> {
            controlButton.setText("<");
            controlButton.getStyleClass().add("hide-left");
...
mediaView.setOnError((MediaErrorEvent mediaErrorEvent)->{
});
return mediaView;
}
